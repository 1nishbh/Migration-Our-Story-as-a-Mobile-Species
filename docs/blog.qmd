...
It was very interesting to see the overall trend since 2000 in migration patterns in the above sections. It would further be interesting to analyze if there are certain years or countries that shares the same pattern on how people move. For this K-means clustering was used from scikit-learn library. It was figured out that the four clusters tend to account the overall variability in the number of moving populations: clusters with lower differences in observations within the same clusters but higher inter-cluster differences.
...
@fig-kmeans shows almost consistent patterns in migrating populations throughout the years. Cluster 0 to CLuster 3 are odered in ascending order, indicating the total number of people moving from a country to another. Years before 2005 have few observations in cluster 3, meaning there were not that many migrations in huge numbers. From 2015-2020 there are some observations that are very different from others, even if they belong to the same cluster 3. Even if we don't see any pattern through cluster 0, 1, and 2, cluster 3 shows some patterns across years: fewer large mirations before 2005, similar large migrations in 2005-2015, and few larger migrations around 2015-2020. 
...
@fig-kmeans_country shows different countries have differing movements over time. Only few countries have observations in cluster 3 which represents largest moving populations. We can look into the clusters that classifiy the movements to countries.
...
ax.set_title('Total Inflows of foreign population')
plt.show()
```

We shown in @fig-schen, throughout the years, there is not much differences in inflows of foreign population among the selected schengen and non-schengen countries. From previous sections, we can say Germany is the schengen country with largest migrations around 2015. The non-schengen country with slightly lower inflowing population is probably United States. 
...
The blog revealed several notable patterns and trends in global migration dynamics over the past two decades. Countries like the United States and Germany consistently emerged as major destinations, attracting significant inflows from a multiple countries. Interestingly, migrants fleeing political unrest and economic instability tended to seek naturalization in their destinations, while migrants from relatively stable countries generally exhibited a higher tendency to return to their home nations.

There is also interlinks between migrations and economic factors, as showed by the strong correlation between remittance inflows and countries with substantial emigrant populations. The analysis of research papers related to migration revealed a predominance of positive sentiments, potentially reflecting a growing recognition of the multifaceted benefits associated with human mobility.

Factors like climate change, conflicts, and shifting economic opportunities are likely to shape future migration trajectories, necessitating adaptive policies and international cooperation to address the multifaceted challenges and opportunities associated with human mobility. Continued research, data collection, and policy dialogue will be essential in navigating the evolving migration landscape and promoting sustainable and equitable outcomes for both migrant populations and host communities.

# Limitations

While this analysis provides insights into global migration patterns and trends, it is important to acknowledge certain limitations in the data and methodology employed. The datasets used in this analysis primarily cover OECD countries and may not capture the full extent of migration dynamics in non-OECD regions. Additionally, data collection during and after the COVID-19 pandemic years appears to be incomplete, potentially affecting the accuracy of recent migration figures.

The data sources utilized in this study do not account for undocumented or irregular migration, which can be a significant component of overall migration flows in certain regions or countries. While the analysis identifies patterns and correlations between migration flows and various factors such as economic conditions, conflicts, and policy changes, it does not establish definitive causal relationships or explore the complex motivations and decision-making processes underlying individual migration decisions.

The analysis primarily focuses on the period from 2000 to 2022, which may not fully capture long-term historical trends or potential future shifts in migration patterns due to evolving global dynamics. Additionally, the analysis primarily focuses on international migration, but does not delve into internal migration dynamics within countries, which can also significantly impact population distributions and socioeconomic factors.

Due to the nature of the data sources, the analysis primarily relies on aggregated migration figures at the national level, potentially neglecting nuances and variations within countries or specific regions. Despite these limitations, the analysis provides valuable insights into global migration trends and highlights areas for further exploration and research. Future studies could benefit from incorporating additional data sources, exploring qualitative factors influencing migration decisions, and employing more granular spatial and temporal analyses to gain a more comprehensive understanding of this complex phenomenon.
